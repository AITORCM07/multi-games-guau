...
func setup_controls():
	if not InputMap.has_action("ui_rotate"):
		InputMap.add_action("ui_rotate")
	...
	if not InputMap.has_action("move_down_custom"):
		...
		s_key.keycode = KEY_S
		InputMap.action_add_event("move_down_custom", s_key)
...
func can_rotate():
	...
	for i in piece_type[temp_index]:
		if not is_free(i + cur_pos):
			return false
	return true

func is_free(pos):
	if pos.x <= 0 or pos.x > COLS or pos.y > ROWS:
		return false
